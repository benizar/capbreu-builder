...
l1_plots_context <- function(nodes.csv, edges.csv, output.html){
  ...
  edges<-
    edges %>% 
    filter(type=='plot-level1-part'|type=='plot-border-l2'|type=='plot-border-l1'|type=='plot-border'|type=='plot-admin-border'|type=='plot-anthropic-border'|type=='plot-rivers-border'|type=='plot-mountains-border') %>% 
    select(from,to,type)
